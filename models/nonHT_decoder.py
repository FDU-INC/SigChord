...
class Net(nn.Module):
    ...
    def __init__(
        ...
        # 2 additional tokens for known 0s and 1s, legacy design
        ...
        self._name = f"nonHT_mm_q_lse_v3_all_L{n_layer}_D{d_model}_C{n_coset}_F{n_fold}"
...
class Loss(nn.Module):
    ...
    def __init__(self) -> None:
        ...
        self.valid_bits_ids = np.concatenate([np.arange(0, 4), np.arange(5, 18)])
        self.valid_bits_ids = list(self.valid_bits_ids)

        orders_2 = torch.arange(self.N_LEN_BIT)
        self.orders_2 = torch.pow(2, orders_2).long()
    ...
    def forward(self, bit_pred: Tensor, bit_gt: Tensor) -> Tensor:
        ...
        bit_loss = self.cross_entropy_loss(bit_pred, bit_gt).mean()
        return bit_loss
    ...
    def stat(self, bit_pred: Tensor, bit_gt: Tensor, verbose=False):
        bit_pred = bit_pred.argmax(dim=-1)

        # sig length bits
        orders = self.orders_2.unsqueeze(0).expand(bit_gt.size(0), -1).to(bit_gt.device)
        len_pred = (bit_pred[:, 5:17] * orders[:, :12]).sum(dim=-1)
        len_gt = (bit_gt[:, 5:17] * orders[:, :12]).sum(dim=-1)
        len_diff_sum = (len_pred - len_gt).abs().sum().item()
        len_gt_sum = len_gt.sum().item()

        # mcs bits
        mcs_pred = bit_pred[:, :4]
        mcs_gt = bit_gt[:, :4]
        mcs_shot = (mcs_pred == mcs_gt).all(dim=-1).sum().item()
        mcs_total = bit_gt.size(0)

        if verbose:
            each_bit = (bit_pred == bit_gt[:, :self.N_BIT]).sum(dim=0)
            self.pkt_len_diff_stat = torch.cat([self.pkt_len_diff_stat, (len_pred - len_gt).cpu()])
            return len_diff_sum, len_gt_sum, mcs_shot, mcs_total, each_bit
        else:
            return len_diff_sum, len_gt_sum, mcs_shot, mcs_total
...
def test(model: Net, loss_fn: Loss, test_dl: DataLoader, snr: float | None = 10, verbose=False):
    ...
    with torch.no_grad():
        for n, (x, bands, bit_gt) in enumerate(test_dl):
            ...
            if verbose:
                len_d, len_tot, mcs, n_sig, each_bit = loss_fn.stat(bit_pred, bit_gt, verbose)
                each_bit_total += each_bit.to(each_bit_total.device)
            else:
                len_d, len_tot, mcs, n_sig = loss_fn.stat(bit_pred, bit_gt, verbose)
            len_diff += len_d
            len_gt += len_tot
            mcs_shot += mcs
            sig_total += n_sig
    ...
    print("length diff:", len_diff / (len_gt + h))
    if verbose:
        print("each bit stat:", each_bit_total / (sig_total + h))
        print("mean bit accuracy: ", torch.mean(each_bit_total / (sig_total + h)))
        np.save(f"./results/nonHT_each_bit_C{model.n_coset}.npy", (each_bit_total / (sig_total + h)).cpu().numpy())
        np.save(f"./results/nonHT_pkt_len_stat_C{model.n_coset}.npy", loss_fn.pkt_len_diff_stat.numpy())
...
def params_test():
    ...
    class Wrapper(nn.Module):
        ...
        def forward(self, x: Tensor):
            bands = torch.randint(-1, 3, (x.size(0), self.model.n_band)).to(x.device)
