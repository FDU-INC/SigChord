...
class Net(nn.Module):
    ...
    def __init__(
        ...
        # 2 additional tokens for known 0s and 1s, legacy designs
        ...
        self.query_tokens = torch.arange(self.N_BIT, dtype=torch.long)

        self._name = f"dvbs2_L{n_layer}_D{d_model}_C{n_coset}_F{n_fold}"

    def pkt_cls_embedding_(self, x: Tensor) -> Tensor:
    ...
    def __pre_input_embedding(self, x: Tensor, bands: Tensor) -> Tensor:
        ...
        pkt_cls_feat = self.pkt_cls_embedding_(x)
    ...
    def decode(self, x: Tensor, pkt_cls_feat: Tensor) -> Tensor:
        ...
        # query = torch.arange(self.N_BIT, dtype=torch.long, device=x.device).unsqueeze(0)
        query = self.query_tokens.expand(x.size(0), self.N_BIT).to(x.device)
...
class Loss(nn.Module):
    ...
    def stat(self, bit_pred: Tensor, bit_gt: Tensor, verbose=False):
        ...
        # frame size
        size_shot = (bit_pred[:, 5] == bit_gt[:, 5]).sum().item()

        # pilot
        pilot_shot = (bit_pred[:, 6] == bit_gt[:, 6]).sum().item()
        total = bit_gt.size(0)

        if verbose:
            each_bit = (bit_pred == bit_gt[:, :self.N_BIT]).sum(dim=0)
            return mcs_shot, size_shot, pilot_shot, total, each_bit
        else:
            return mcs_shot, size_shot, pilot_shot, total
...
def test(model: Net, loss_fn: Loss, test_dl: DataLoader, snr: float | None = 10, verbose=False):
    ...
    with torch.no_grad():
        for n, (x, bands, bit_gt) in enumerate(test_dl):
            ...
            if verbose:
                mcs, size, pilot, n_sig, each_bit = loss_fn.stat(bit_pred, bit_gt, verbose)
                each_bit_total += each_bit.to(each_bit_total.device)
            else:
                mcs, size, pilot, n_sig = loss_fn.stat(bit_pred, bit_gt, verbose)
            mcs_shot += mcs
            size_shot += size
            pilot_shot += pilot
            sig_total += n_sig
    ...
    print("size acc:", size_shot / (sig_total + h))
    print("pilot acc:", pilot_shot / (sig_total + h))

    if verbose:
        print("each bit stat:", each_bit_total / (sig_total + h))
        np.save(f"./results/dvbs2_each_bit_C{model.n_coset}.npy", (each_bit_total / (sig_total + h)).cpu().numpy())

    test_acc = round(test_bit_shot / (test_bit_total + H), 4)
    print(".......test loss {}, acc {}".format(test_loss, test_acc), flush=True)
